module.exports = function(db, bot, config, links, send, logger, serversArray, channels, playingArray) {
    var serverCommands = [{
        command: "RESTARTMAP",
        fn: restartMap
    }, {
	...
	function updatePlayingArray(newPlayingArray){
		playingArray=newPlayingArray;
logger.info('updated player array');
		return;
	}
    ...
    function matchEnded(data, serverI, to) {
        ...
        if (data[0] === "Blue") {
            ...
            for (var j = 0; j < playingArray.redTeam.length; j++) {
                db.getPlayerByAuth(playingArray.redTeam[j], function(result) {
                    send(serverI, "/kick " + result.name);
                }); // jshint ignore:line
...
logger.info("checking the value of playingarray: "+playingArray+" blueteam: "+playingArray.blueTeam[0]);
    }
