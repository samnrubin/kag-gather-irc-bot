...
module.exports = function(db, bot, config, links, send, logger, serversArray, channels) {
    ...
    function matchEnded(data, serverI, to) {
        var whoWon;
        logger.info("checking the value of arrayObject.playingArray: "+arrayObject.playingArray+" blueteam: "+arrayObject.playingArray.blueTeam);
        if (data[0] === "Blue") {
...
logger.info("checking the value of arrayObject.playingArray: "+arrayObj.playingArray+" blueteam: "+arrayObj.playingArray.blueTeam);
    ...
    function requestSub(data, serverI) {
        ...
        if (arrayObject.subsArray[0]) {
            db.getPlayerByAuth(arrayObject.subsArray[0].account, function(result) {
                send(serverI, "/assignseclev " + result.name + " " + seclevID);
            });
            bot.say(arrayObject.subsArray[0].nick, "You are now playing for gather in place of " + subbed + ". The server's IP:Port : " + serversArray[serverI].publicIp + ":" + serversArray[serverI].port);
            bot.say(channels,arrayObject.subsArray[0].nick+ "subbed into the match");
            ...
            db.getPlayerByAccount(subbed, function(result) {
                ...
                if (team === "blue") {
                    playingArray.blueTeam.push(arrayObject.subsArray[0]);
                } else {
                    playingArray.redTeam.push(arrayObject.subsArray[0]);
                }
                arrayObject.subsArray.splice(0, 1);
            });
        }
