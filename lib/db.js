module.exports = function(config, logger) {
    ...
    return {
        //temp links
        tempLinkData: [],
        addTempLinkData: function(nick, kagUsername){
        ...
        getTempKagNameByNick: function(nick){
            for (var i = 0; i < this.tempLinkData.length; i++) {
                if (this.tempLinkData[i].nick == nick) {
                    //overwrite old data
                    return this.tempLinkData[i].kagUsername;
                }
            }
            return undefined;
        },

        getTempNickByKagName: function(kagName){
            for (var i = 0; i < this.tempLinkData.length; i++) {
                if (this.tempLinkData[i].kagUsername == kagName) {
                    //overwrite old data
                    return this.tempLinkData[i].nick;
                }
            }
            return undefined;
        },
            ...
            pool.getConnection(function(err, connection) {
                ...
                connection.query("SELECT stats FROM " + usersTable + " WHERE authname=? LIMIT 1;", [account], function(err, result) {
                    ...
                    connection.query("UPDATE " + usersTable + " SET stats=? WHERE authname=?;", [stats, account], function(err) {
                        if (err) throw err;
                        logger.info('Added victory to "' + account + '"');
                        connection.release();
                        //cb();
                    });
                });
            ...
            pool.getConnection(function(err, connection) {
                ...
                connection.query("SELECT stats FROM " + usersTable + " WHERE authname=? LIMIT 1;", [account], function(err, result) {
                    ...
                    connection.query("UPDATE " + usersTable + " SET stats=? WHERE authname=?;", [stats, account], function(err, result) {
                        if (err) throw err;
                        logger.info('Added lose to "' + account + '"');
                        connection.release();
                        //cb();
                    });
                });
