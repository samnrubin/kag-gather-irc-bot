module.exports = function(config, logger) {
    ...
    return {
        ...
        addTempLinkData: function(nick, kagUsername, callback){
            pool.getConnection(function(err, connection) {              //code to check for account already being used taken from 
                connection.query("SELECT authname FROM " + usersTable + " WHERE name=? LIMIT 1;", [name], function(err, result) {
                    ...
                    if (result[0]) {
                        ...
                        if(this.tempLinkData){          //account not in use, can add a temp link for it
