module.exports = function(db, bot, config, send) {
    ...
    function startMatch(to) {
        ...
        getTeams(function(blueTeam, redTeam) {
            ...
            for (var i = 0; i < playersArray.length; i++) {
                bot.say(playersArray[i].nick, "You are now playing for gather.");
            };
            console.log('STARTING MATCH: ', blueTeam, redTeam);
            playingArray = {
                blueTeam: blueTeam,
                redTeam: redTeam
            }
            console.log('STARTING 222 MATCH: ', playingArray);
            playersArray = [];
            isPlaying = true;
            console.log(playingServer, serversArray);
