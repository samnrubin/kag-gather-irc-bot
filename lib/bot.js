...
module.exports = function(db, bot, config, links, playerManagement, send, logger) {
    var commands = [{
        command: "!add",
        fn: addHandler,
        adminOnly: false,
    ...
    function addPlayerToQueue(list, account, nick, host, vote) {
        if (list === "sub-list") {
            arrayObject.subsArray.push({
                account: account,
                nick: nick,
                host: host
            });
            logger.info('Added "' + nick + '" to the sub-list.');
            return arrayObject.subsArray;
        } else {
            playersArray.push({
                account: account,
                nick: nick,
    ...
    function makeSub(from, to, message){
        ...
        bot.whois(from, function(WHOIS) {
            db.getPlayerByAuth(WHOIS.account, function(player) {
                if(playermanagement.makeSub(player);
                    bot.say(channels, from + " has subbed into the match!");
                else
                    bot.say(to, from ": There is no sub position availible");
            });
        }
    }
...
logger.info('playingarray: '+arrayObject.playingArray+'playingarray.blueTeam: '+arrayObject.playingArray.blueTeam+'playingarraylength: '+arrayObject.playingArray.length+'playingarray.blueteam.length: '+arrayObject.playingArray.blueTeam.length);
/*
            var blueTeamNicks = [];
            var redTeamNicks = [];

            for(var i=0;i<arrayObject.blueTeam.length;i++){
                blueTeamNicks[i]=arrayObject.blueTeam[i].nick;
            }
            for(var i=0;i<arrayObject.redTeam.length;i++){
...
*/
    ...
    function removePlayerFromQueue(from, to) {
        bot.whois(from, function(WHOIS) {
            ...
            if (isPlaying) {
                ...
                if(arrayObject.subsArray != undefined){
                    for (var i = 0; i < arrayObject.subsArray.length; i++) {
                        if (arrayObject.subsArray[i].account === account) {
                            arrayObject.subsArray.splice(i, 1);
                            removed = true;
    ...
    function playerIsInQueue(account) {
        ...
        if (isPlaying) {
            ...
            if(arrayObject.playersArray!=undefined){
                for (var i = 0; i < playersArray.length; i++) { // jshint ignore:line
                    if (playersArray[i].account === account) {
                        is = true;
                    }
    ...
    function playerIsPlaying(account) {
        ...
        for (var i = 0; i < arrayObject.playingArray.blueTeam.length; i++) {
            logger.info('playerIsPlaying?: '+account+' compare to: player in list: '+arrayObject.playingArray.blueTeam[i].account+' nick: '+arrayObject.playingArray.blueTeam[i].nick+' host: '+arrayObject.playingArray.blueTeam[i].host);
            if (arrayObject.playingArray.blueTeam[i] === account) {
                return true;
            }
        }
        for (var i = 0; i < arrayObject.playingArray.redTeam.length; i++) {
            logger.info('playerIsPlaying?: '+account+' compare to: player in list: '+arrayObject.playingArray.redTeam[i].account+' nick: '+arrayObject.playingArray.redTeam[i].nick+' host: '+arrayObject.playingArray.redTeam[i].host);
            if (arrayObject.playingArray.redTeam[i] === account) {
                return true;
            }
        }
        return false;
    ...
    function showList(from, to) {
        if (isPlaying) {
            var list = [];
            for (var i = 0; i < arrayObject.subsArray.length; i++) {
                list.push(arrayObject.subsArray[i].nick);
            }
    ...
    function forceRemove(from, to, message) {
        ...
        if (msg[1]) {
            ...
            for (var j = 0; j < arrayObject.subsArray.length; j++) {
                if (arrayObject.subsArray[j].account === authname) {
                    arrayObject.subsArray.splice(j, 1);
                    bot.say(to, from + " was removed from the sub-list");
                    return;
                }
            }
    ...
    function onPart(a, b, c, raw) {
        ...
        for (var j = 0; j < arrayObject.subsArray.length; j++) {
            if (arrayObject.subsArray[j].host === raw.host) {
                arrayObject.subsArray.splice(j, 1);
                bot.say(channels, raw.nick + " was removed from the queue(left IRC).");
            }
        }
    ...
    function onNick(oldNick, newNick) {
        ...
        for (var j = 0; j < arrayObject.subsArray.length; j++) {
            if (arrayObject.subsArray[j].nick === oldNick) {
                arrayObject.subsArray[j].nick = newNick;
            }
        }
    }
    return {
