...
module.exports = function(db, bot, config, links, playerManagement, connectedArray, send, logger, channels) {
    var commands = [{
        command: "!whatis",
        ...
        fn: addEndVote,
        adminOnly: false,
        description: "Adds vote to end a match",
        usage: "!end"
    }, {
        command: "!username",
        fn: getUserUsername,
...
//logger.info('playermgmt: '+playermgmt2);
        if(playerManagement.isPlaying) return;   //temp check so nobody added to sublist
        canUserAdd(from, function(response) {
            if (response.can) {
                ...
                if (playersArray.length === playersNeeded) {
                    ...
                    bot.say(to, from+": the bot is not currently connected your server choice, if a conection cannot be reestablished before the game starts it will not be chosen");
                }
            } else {
...
logger.info("a vote is: "+playersArray[i].vote);
...
logger.info("votes count: ["+k+"] "+votes[k].count);
...
logger.info("most voted: "+mostVotedID+" count: "+mostVotesCount);
            for(var i=0;i<serversArray.length;i++){
                if(serversArray[i].name==mostVotedName)
                    mostVotedID=i;
            }

            if(connectedArray[mostVotedID]) {
                break;
            } else {
                bot.say(channels, "no connection with server "+mostVotedName+"choosing next most voted");
                votes.splice(votesI, 1);            //delete server from vote options
            }
        }
...
return; //temporary, make sure not check sub list
    ...
    function getSubs(from, to, message){
        if(playerManagement.subRequests && playerManagement.subRequests.length>0){
            bot.say(from, "sub positions available to replace: "+playerManagement.subRequests.KAGName+" on server: "+serversArray[playerManagement.playingServer].name);
        }else{
            ...
            //bot.whois(from, function(WHOIS) {
                //db.getPlayerByAuth(WHOIS.account, function(player) {
                    if(playerManagement.makeSub(kagName)){
                        bot.say(channels, from + " has subbed into the match on the "+serversArray[playerManagement.playingServer].name+" server!  kag://"+serversArray[playerManagement.playingServer].publicIp+":"+serversArray[playerManagement.playingServer].port);
                        sendUpdatedTeams();
                    }else{
...
logger.info("get name by nick kagName: "+kagName);
            if(kagName!=undefined){
                callback(kagName);
            }else{
...
//this code wont work, no way to get nick from auth without having a userlist
    ...
    function startMatch(to) {
        ...
        playerManagement.playingServer = getMostVotedServer();
...
//logger.info("blue team: "+blue);
            ...
            for (var i = 0; i < playersArray.length; i++) {
                bot.say(playersArray[i].nick, "You are now playing for gather on the " + serversArray[playerManagement.playingServer].name + " server.");
            }
...
logger.info('playingarray: '+playerManagement.playingArray+'playingarray.blueTeam: '+playerManagement.playingArray.blueTeam+'playingarraylength: '+playerManagement.playingArray.length+'playingarray.blueteam.length: '+playerManagement.playingArray.blueTeam.length);
            ...
            bot.say(to, "Match started on server " + serversArray[playerManagement.playingServer].name + ": " + blueTeam + "(Blue Team) VS " + redTeam + " (Red Team)" + " kag://"+serversArray[playerManagement.playingServer].publicIp +":"+serversArray[playerManagement.playingServer].port );
        });
    }
...
logger.info("Showing player list for "+from+" to "+to);
    ...
    function showHelp(from) {
        bot.say(from, "In order to play you must be authed and/or linked. Follow this guide to set up an auth account: https://www.quakenet.org/help/q/how-to-register-an-account-with-q .  You can link using !link <kagUsername> (!link will create a temporary account if you are not authed).  More help and information on how to play gather can be found at: "+ guideLink);
...
logger.info("msg[1]= "+msg[1]);
    ...
    function showConnectedServers(from, to){
        bot.say(from, "Server connection status is: "+connectedArray);
    }
        ...
        bot.whois(from, function(WHOIS) {
            if (WHOIS.account) {
                ...
                db.addTempLinkData(WHOIS.nick, username, function(returnVal) {
                    if(!returnVal) bot.say(to, from + ": could not create temporary link, the KAG account is already in use");
                    else bot.say(to, from + ": You are not authed, so a temporary link was made for you, !help for more info.  You may now add using !add");
                }); 
            }
        });
    }
