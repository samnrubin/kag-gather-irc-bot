...
logger.info("can add kagname: "+kagName);
    	        if(kagName==undefined){
                    if(WHOIS.account){          //they are authed
                        callback({
                            can: false,
                            message: userNick + ": you must link your irc account to your kag account using !link <kagUsername> (no <>)",
                            user: WHOIS,
                            kagName : kagName
                        });
                    }else{
                	    callback({
                            can: false,
                            message: userNick + ": you must be linked or authed to play, type !help for help authing or you can link using !link <kagUsername> (no <>)",
                            user: WHOIS,
                            kagName : kagName
        	            });
                    }
            ...
            //bot.whois(from, function(WHOIS) {
                //db.getPlayerByAuth(WHOIS.account, function(player) {
                    if(playerManagement.playerIsPlaying(kagName){
                        ...
                        return;
                    }else if(playerManagement.makeSub(kagName)){
...
logger.info("Showing player list for "+from+" to "+to);
        if (playerManagement.isPlaying) {
            ...
            bot.say(from, "Game is aready running on the "+serversArray[playerManagement.playingServer].name+" server "+playerManagement.playingArray.blueTeam +" vs "+playerManagement.playingArray.redTeam+serverLink+subRequired);
            /*var list = [];
            for (var i = 0; i < playerManagement.subsArray.length; i++) {
