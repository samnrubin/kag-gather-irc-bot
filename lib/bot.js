...
module.exports = function(db, bot, config, links, playerManagement, send, logger, irc, channels) {
    ...
    var commands = [{
        ...
        fn: moveServer,
        adminOnly: false,
        description: "moves server or starts server move vote if not admin",
        usage: "!moveserver <from server> <to server>"
    }, {
        command: "!refreshmatches",
        fn: refreshMatches,
        adminOnly: false,
        description: "refreshes matches on the bot if bot has no matches running or server if bot knows what is running",
        usage: "!refreshmatches"
    }];
    var playersArray = [];
    //var subsArray = [];
    ...
    var giveWinVotesRequired = 6;
    var defaultServer = 0;
    var moveServerVotesRequired=7;
    ...
    /*function getNickFromKagName(kagName){
        ...
        if(nick!=undefined){
            ...
            db.getPlayerByAccount(kagName, function(player) {
                ...
                function addQuery(nick, i, message) {
logger.info("nick: "+nick+" i "+i);
                    setTimeout(function(nick, i, message) {
logger.info("nick: "+nick+" i "+i);
                        bot.say(nick,message);
                    }(nick,i, message), (i+1)*1000);
                }
...
*/
                var message = "you can join the game using http://furai.pl/joingame/"+serversArray[playerManagement.playingServer].publicIp +"/"+serversArray[playerManagement.playingServer].port;
                if(serversArray[playerManagement.playingServer].password){
                    message = message+"/"+serversArray[playerManagement.playingServer].password;
                }
                addQuery(playersArray[i].nick, i,message);
            }
...
logger.info('playingarray: '+playerManagement.playingArray+'playingarray.blueTeam: '+playerManagement.playingArray.blueTeam+'playingarraylength: '+playerManagement.playingArray.length+'playingarray.blueteam.length: '+playerManagement.playingArray.blueTeam.length);
            ...
            if(serversArray[playerManagement.playingServer].password){
                bot.say(to, playerNicks+" Match started on server " + serversArray[playerManagement.playingServer].name + ": " + blueTeam + "(Blue Team) VS " + redTeam + " (Red Team)" /*+ " join with http://furai.pl/joingame/"+serversArray[playerManagement.playingServer].publicIp +"/"+serversArray[playerManagement.playingServer].port+"/"+serversArray[playerManagement.playingServer].password*/);
            }else{
                bot.say(to, playerNicks+" Match started on server " + serversArray[playerManagement.playingServer].name + ": " + blueTeam + "(Blue Team) VS " + redTeam + " (Red Team)" /*+ " join with http://furai.pl/joingame/"+serversArray[playerManagement.playingServer].publicIp +"/"+serversArray[playerManagement.playingServer].port*/ );
            }
        });
    }

    function doMoveServer(newServer){
        playerManagement.playingServer=newServer;
        sendTeams();
        send(playerManagement.playingServer, "getRules().set_bool('teamsSet',true);");
        playerManagement.moveServerVotes=[];
    }

    function moveServer(from, to, message){
        if(!playerManagement.isPlaying) {
            bot.say(to, from + ": Cannot move server when no game running");
            return;
        }
        ...
        if (vote != null){
            vote=vote.toUpperCase();
            server=isValidVote(vote);
        }
        if (server==-1 || vote==null) {
            bot.say(to, from + ": '" + vote + "' is an unknown server name. Use !servers to get the server name you want.");
            return;
        }
        ...
        getKagNameByNick(from, function(kagName) {
            if(!kagName) {
                bot.say(to, from+": you must be linked or authed and linked to do that");
                return;
            }
            bot.whois(from, function(WHOIS) {
                if(isAdmin(WHOIS.account)){
                    doMoveServer(server);
                    bot.say(to,"match moved to "+vote+" by "+from);
                    logger.info("match moved to "+vote+" by "+from);
                    return;
                }else{
                    for(var i=0;i<playerManagement.moveServerVotes.length;i++){
                        if(playerManagement.moveServerVotes[i].kagName==kagName) {
                            playerManagement.moveServerVotes[i].serverVote=server;
                            bot.say(to,from +": your vote to move server was changed to "+vote);
                            return;
                        }
                    }
                    //no existing vote
                    playerManagement.moveServerVotes.push({
                        kagName : kagName,
                        serverVote : server
                    });
                    bot.say(to, from +": vote to move server counted ["+playerManagement.moveServerVotes.length+"/"+moveServerVotesRequired+"]");
                    if(playerManagement.moveServerVotes.length>=moveServerVotesRequired){
                        doMoveServer(server);
    ...
    function refreshMatches(from, to, message){
    ...
    function removePlayerFromQueue(from, to, kagName) {
        ...
        if (playerManagement.playerIsPlaying(kagName)) {
            ...
            if(playerManagement.addSubRequest(kagName)){
                ...
                bot.say(to, "cannot sub player, they are not in the game");
            }
        } else if(playerIsInQueue(kagName)){
            for (var i = 0; i < playersArray.length; i++) { 
    ...
    function showPlayerStats(from, to, message) {
        ...
        db.getPlayerStats(player, function(stats) {
            if (stats === "player-no-exists") {
                bot.say(to, from + ": this player doesn't exist, usage is !stats authName, stats for players without an auth are not recorded");
            } else {
                stats = stats.split(",");
...
logger.info("msg: "+msg);
    ...
    function addEndVote(from, to) {
        ...
        getKagNameByNick(from, function(kagName) {
            for(var i=0;i<playerManagement.endVotes.length;i++){
                if(playerManagement.endVotes[i]==kagName) {
            ...
            if(playerManagement.endVotes.length>=endVotesRequired){
                ...
                bot.say(to, "vote to force end the match counted for "+from+" ["+playerManagement.endVotes.length+"/"+endVotesRequired+"]");
            }
        });
    }
    ...
    function addSubVote(from, to, message) {
        ...
        if(!playerToSub){
            ...
            return;
        }else if(!playerManagement.playerIsPlaying(playerToSub)){
            bot.say(to, "this player is not in the match (case sensitive?)");
            return;
        }else if(!playerManagement.isPlayerBeingSubbed(playerToSub)){
            bot.say(to, "this player is already being subbed");
            return;
        }
...
logger.info("msg[1]= "+msg[1]);
                ...
                bot.whois(from, function(WHOIS) {
                    if(isAdmin(WHOIS.account)){
                        ...
                        //check if voted for any team yet
                            for(var i=0;i<playerManagement.giveWinBlueVotes.length;i++){
                                if(playerManagement.giveWinBlueVotes[i]==kagName) {
                                    playerManagement.giveWinBlueVotes.splice(i,1);
                                    bot.say(to,from +": your previous vote was removed");
                                }
                            }
                            for(var i=0;i<playerManagement.giveWinRedVotes.length;i++){
                                if(playerManagement.giveWinRedVotes[i]==kagName) {
                                    playerManagement.giveWinRedVotes.splice(i,1);
                                    bot.say(to, from +": your previous vote was removed");
                                }
                            }
                        //}
                        if(teamNum==0){
                            playerManagement.giveWinBlueVotes.push(kagName);
                            if(playerManagement.giveWinBlueVotes.length>=giveWinVotesRequired){
                                db.addResultsToDB(0);
                                playerManagement.endMatch();
                                bot.say(to,"match ended with win given to blue team");
                                logger.info("match ended with win given to blue team");
                            }else{
                                bot.say(to, "vote to give win counted for blue ["+playerManagement.giveWinBlueVotes.length+"/"+giveWinVotesRequired+"]");
                            }
                        }else if(teamNum==1){
                            playerManagement.giveWinRedVotes.push(kagName);
                            if(playerManagement.giveWinRedVotes.length>=giveWinVotesRequired){
                                db.addResultsToDB(1);
                                playerManagement.endMatch();
                                bot.say(to,"match ended with win given to red team");
                                logger.info("match ended with win given to red team");
                            }else{
                                bot.say(to, "vote to give win counted for red ["+playerManagement.giveWinRedVotes.length+"/"+giveWinVotesRequired+"]");
                            }
                        }
                    }
                });
    ...
    function requestIRCLink(from, to, message) {
        ...
        if(playerIsInQueue(kagName)){
            bot.say(to, from + ": you must leave the queue to create or change a link");
            return;
        }else if(playerManagement.playerIsPlaying(kagName) && playerManagement.isPlayerBeingSubbed()==-1){
            bot.say(to, from + ": you cannot change your link while in a game");
            return;
        }
