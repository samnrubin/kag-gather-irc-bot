...
//logger.info('playermgmt: '+playermgmt2);
        ...
        canUserAdd(from, function(response) {
            if (response.can) {
                ...
                addPlayerToQueue(response.user.account, response.user.nick, response.user.host, vote);
    ...
    function addPlayerToQueue(account, nick, host, vote) {
/*        if (list === "sub-list") {
...
*/
            ...
            if (isPlaying) {
                ...
                if (playerIsPlaying(account)) {
                    ...
                    db.getPlayerByAuth(WHOIS.account, function(player) {
                        ...
                        playerManagement.addSubRequest(player);
                        bot.say(to, "A sub is needed for the game running on server: "+serversArray[playingServer].name);
                        bot.say(to, "You may sub into the match using !sub");
    ...
    function playerIsPlaying(account) {
        ...
        for (var i = 0; i < playerManagement.playingArray.blueTeam.length; i++) {
            logger.info('playerIsPlaying?: '+account+' compare to: player in list: '+playerManagement.playingArray.blueTeam[i].account+' nick: '+playerManagement.playingArray.blueTeam[i].nick+' host: '+playerManagement.playingArray.blueTeam[i].host);
            if (playerManagement.playingArray.blueTeam[i] === account) {
                return true;
            }
        }
        for (var i = 0; i < playerManagement.playingArray.redTeam.length; i++) {
            logger.info('playerIsPlaying?: '+account+' compare to: player in list: '+playerManagement.playingArray.redTeam[i].account+' nick: '+playerManagement.playingArray.redTeam[i].nick+' host: '+playerManagement.playingArray.redTeam[i].host);
            if (playerManagement.playingArray.redTeam[i] === account) {
                return true;
            }
        }
        return false;
