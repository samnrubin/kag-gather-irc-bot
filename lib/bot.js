...
return; //temporary, make sure not check sub list
    ...
    function sendUpdatedTeams(){
        ...
        teamString=teamString+"'"+playerManagement.playingArray.blueTeam[blueTeam.length-1]+"'}; ";
        ...
        teamString=teamString+"'"+playerManagement.playingArray.redTeam[redTeam.length-1]+"'}; ";
    ...
    function makeSub(from, to, message){
        ...
        bot.whois(from, function(WHOIS) {
            db.getPlayerByAuth(WHOIS.account, function(player) {
                if(playerManagement.makeSub(player.name)){
