...
return; //temporary, make sure not check sub list
    ...
    function getSubs(from, to, message){
        if(playerManagement.subRequests && playerManagement.subRequests.length>0){
            bot.say(from, "sub positions available to replace: "+playerManagement.subRequests.KAGName+" on server: "+serversArray[playingServer].name);
        }else{
            ...
            //bot.whois(from, function(WHOIS) {
                //db.getPlayerByAuth(WHOIS.account, function(player) {
                    if(playerManagement.makeSub(kagName)){
                        bot.say(channels, from + " has subbed into the match on the "+serversArray[playingServer].name+" server!  kag://"+serversArray[playingServer].publicIp+":"+serversArray[playingServer].port);
                        sendUpdatedTeams();
                    }else{
...
logger.info("Showing player list for "+from+" to "+to);
    ...
    function endMatch(winner){
        //if(!winner) //no winner given, match was force ended
        return;
    }
    ...
    function forceMatchEnd(from, to) {
        ...
        playerManagement.playingArray= { 
            ...
            playerManagement.subRequests= []
            playerManagement.subVotes= [{
                kagNamePlayerToSub: "",
    ...
    function addEndVote(from, to) {
        ...
        if(endVotes.length>=endVotesRequired){
            bot.say(to, "Forced end on server " + playingServer);
            playingServer = null;
            isPlaying = false;
            playerManagement.playingArray= { 
                blueTeam: [],
            ...
            logger.info('Forced match end on ' + playingServer);
        }else{
    ...
    function giveWin(from, to, message) {
        ...
        if (msg[1] === "blue") teamNum=0;
            ...
            bot.say(from, "Invalid input for !givewin <teamname>");
            return
        }
                ...
                if(teamNum==0){
                    for(var i=0;i<giveWinBlueVotes.length;i++){
                        if(giveWinBlueVotes[i]==kagName) {
                            bot.say(to,from +": you have already voted");
                            return;
                        }
                    }
