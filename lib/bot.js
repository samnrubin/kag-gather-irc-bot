            ...
            //bot.whois(from, function(WHOIS) {
                //db.getPlayerByAuth(WHOIS.account, function(player) {
                    if(playerManagement.playerIsPlaying(kagName) && playerManagement.isPlayerBeingSubbed(kagName)==-1){
                        ...
                        if(subTeam!=-1){
                            bot.say(channels, from + " has subbed into the match for "+subTeam+" team on the "+serversArray[playerManagement.playingServer].name+" server!  kag://"+serversArray[playerManagement.playingServer].publicIp+":"+serversArray[playerManagement.playingServer].port);
                            sendUpdatedTeams();
                        }else{
