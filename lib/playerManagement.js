...
module.exports = {
	...
	getPlayerTeam: function(kagName){
        ...
        playerIsPlaying: function(kagName){
        	for (var i = 0; i <this.playingArray.blueTeam.length; i++) { 
console.log("checking if playing: "+kagName+" player in playing array: "+this.playingArray.blueTeam[i]);
			if(this.playingArray.blueTeam[i] == kagName){
...
console.log("checking if playing: "+kagName+" player in playing array: "+this.playingArray.redTeam[i]);
...
console.log("returning true");
	...
	addSubRequest: function(KAGNameToSub){
		if(!this.playerIsPlaying(KAGNameToSub)) return false;
	...
	isPlayerBeingSubbed: function(KAGName){
		...
		for(var i=0;i<this.subRequests.length;i++){
			if(this.subRequests[i].KAGName==KAGName) return i;
		}
		return -1;
	},
...
console.log("this.subRequests[0].team: "+this.subRequests[0].team);
			...
			if(!this.subRequests) return -1;
			for(var i=0;i<this.subRequests.length;i++){
				if(this.subRequests[i].KAGName==KAGNameOfPlayerToSubIn){
					playerIndex = i;
					break;
				}
			}
...
console.log("checking playing array red : "+i+" : "+this.playingArray.redTeam[i]);
	...
	endMatch: function(){
		...
		this.endVotes = [];
		this.giveWinBlueVotes = [];
		this.giveWinRedVotes = [];
		this.moveServerVotes = [];
		return;
	}
}
