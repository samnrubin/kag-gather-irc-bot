...
console.log("checking playing array red : "+i+" : "+this.playingArray.redTeam[i]);
	...
	endMatch: function(winner){
		...
		if (winner == 0) {
			...
			for (var i = 0; i < this.playingArray.blueTeam.length; i++) {
				db.addVictoryTo(this.playingArray.blueTeam[i]);
			}
			for (var j = 0; j < this.playingArray.redTeam.length; j++) {
				db.addLoseTo(this.playingArray.redTeam[j]);
			}
			db.addMatchToDB(this.playingArray.blueTeam, this.playingArray.redTeam, winner);
		} else if(winner == 1){
			//bot.say(to, "Match ended on server " + serverI + ". The red team has won. You can now add to the list.");
			for (var i = 0; i < this.playingArray.blueTeam.length; i++) { // jshint ignore:line
				db.addLoseTo(this.playingArray.blueTeam[i]);
			}
			for (var j = 0; j < this.playingArray.redTeam.length; j++) { // jshint ignore:line
				db.addVictoryTo(this.playingArray.redTeam[j]);
			}
			db.addMatchToDB(this.playingArray.blueTeam, this.playingArray.redTeam, winner);
		}
		//always do this part
